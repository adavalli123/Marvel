...
extension EndPoint {
    ...
    private func fullPath(_ path: String, offset: Int = 0) -> String {
        ...
        if offset == 0 {
            return "\(path)?ts=\(timeStamp)&apikey=\(Person.aravind.publicKey)&hash=\(Security.md5Hash(timeStamp: timeStamp))&limit=10"
        }
        return "\(path)?ts=\(timeStamp)&apikey=\(Person.aravind.publicKey)&hash=\(Security.md5Hash(timeStamp: timeStamp))&limit=10&offset=\(offset)"
    }
}
